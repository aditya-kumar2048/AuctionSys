...
function AuctionGrid({ owners, players, gridData, onViewHistory }) {
  ...
  return (
    <div className="h-full w-full bg-slate-950 font-sans text-slate-200 overflow-hidden flex flex-col border border-slate-800/50 rounded-xl relative" ref={container}>
      ...
      <div className="overflow-x-auto overflow-y-auto h-full relative z-10 custom-scrollbar">
        <table className="w-full text-left border-collapse min-w-max text-sm tabular-nums">
          <thead className="sticky top-0 z-30 shadow-md">
            <tr>
              <th className="px-6 py-4 border-b border-r border-slate-800 backdrop-blur-xl bg-slate-950/90 sticky left-0 z-40 w-72 font-semibold text-slate-400 uppercase tracking-widest text-[10px]">
                Players / Entities
              </th>
              {owners.map(owner => (
                <th 
                  key={owner._id} 
                  className={`px-4 py-4 border-b border-r border-slate-800/50 backdrop-blur-xl bg-slate-950/80 font-medium text-center min-w-[140px] transition-colors duration-300 ${hoveredCol === owner._id ? 'bg-slate-800/50' : ''}`}
                  onMouseEnter={() => setHoveredCol(owner._id)}
                  onMouseLeave={() => setHoveredCol(null)}
                >
                  <div className="flex flex-col items-center gap-1.5">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-700 to-slate-800 border border-slate-600 flex items-center justify-center text-xs font-bold text-slate-300 shadow-sm">
                      {owner.name.substring(0, 2).toUpperCase()}
                    </div>
                    <div className="text-slate-200 truncate w-full text-xs font-semibold">{owner.name}</div>
                    <div className="text-[10px] text-emerald-400/80 font-mono tracking-tight bg-emerald-400/5 px-2 py-0.5 rounded-full border border-emerald-400/10">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {players.map((player) => {
              ...
              return (
                ...
                >
                  <td className={`px-6 ${paddingClass} sticky left-0 z-20 ${nameColClasses} border-b border-slate-800/30`}>
                    <div className="flex items-center gap-4">
                      {player.photo ? (
                        <div className="relative">
                          <img src={player.photo} alt={player.name} className="w-10 h-10 rounded-full object-cover border border-slate-700 shadow-lg" />
                          {isActiveRow && (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className={`font-semibold truncate ${isActiveRow ? 'text-white text-base' : 'text-slate-300'}`}>
                          {player.name}
                        </div>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          <span className="px-2 py-0.5 bg-slate-800 border border-slate-700 rounded text-[10px] text-slate-400 font-medium">
                  ...
                  {owners.map(owner => {
                    ...
                    if (isHighestActive) {
                      ...
                      textClasses = "text-emerald-400 font-semibold text-xl drop-shadow-[0_0_4px_rgba(52,211,153,0.3)]";
                    } else if (isLoser) {
                      cellClasses += " ";
                    ...
                    return (
                      <td key={`${player._id}-${owner._id}`} className={cellClasses}>
                        ...
                        {isWinner && (
                          <div className="absolute inset-0 border border-emerald-500/30 bg-emerald-500/5 pointer-events-none"></div>
                        )}
                        {bid ? (
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${styles.bg} ${styles.border} ${styles.text}`}>
