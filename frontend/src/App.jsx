...
function AppLayout({ owners, players, gridData, activePlayer, historyModalPlayer, setHistoryModalPlayer, fetchData, handleDeleteOwner, handleDeletePlayer }) {
  const location = useLocation();
  const isLive = location.pathname === '/live';

  return (
    <div className={`min-h-screen font-sans flex flex-col transition-colors duration-500 ${isLive ? 'bg-slate-950 text-slate-200 selection:bg-blue-900/30' : 'bg-gray-50 text-gray-900 selection:bg-blue-100'}`}>
      {!isLive && <TopNav />}
        ...
        <Routes>
          <Route path="/live" element={
            <div className="h-full w-full py-4 relative">
              <Link to="/" className="absolute top-0 right-4 p-2 bg-slate-800 text-slate-400 rounded-lg hover:text-white transition shadow z-50">
                 Exit Live Mode
              </Link>
          ...
          <Route path="/" element={
            <div className="flex flex-col lg:flex-row gap-3 h-[calc(100vh-8rem)]">
              <div className="flex-2 flex flex-col min-w-0">
                ...
                <div id="live-grid-container" className="flex-1 overflow-hidden p-0 bg-white items-center h-full">
          ...
          <Route path="/admin" element={
            <div className="flex flex-col gap-6">
              ...
              <AdminControls owners={owners} players={players} activePlayer={activePlayer} onUpdate={fetchData} />
              ...
              <div className=' w-full justify-center flex grow gap-5'>
                <div className='flex flex-col w-1/2 gap-5 rouned-xl bg-white border border-gray-400 rounded'>
                    ...
                    {owners.map((owner) => (
                      <div key={owner._id} className=' py-2 px-3 bg-[#f9fafb] border border-gray-300 rounded-lg w-full flex justify-between hover:bg-gray-300 hover:text-white transition-all duration-300'>
                        <div className=' w-1/2 flex  item-center gap-3'>
                          <div className='w-10 h-10 font-bold text-gray-500 flex justify-center items-center border rounded-full overflow-hidden bg-white'>
                            {owner.photo ? (<img src={owner.photo} alt="" className="w-full h-full object-cover" />) : (owner.name.charAt(0).toUpperCase())}
                          </div>
                        ...
                        <div className='flex justify-center items-center gap-1 text-red-500 font-semibold hover:text-red-700 hover:bg-red-100 hover:border-red-300 hover:border rounded-lg px-2 py-1 transition-all duration-300' onClick={() => handleDeleteOwner(owner._id)}>
                          <Trash2 />
                        </div>
                      </div>
                    ))}
                    ...
                    {players.map((player) => (
                      <div key={player._id} className=' py-2 px-3 bg-[#f9fafb] border border-gray-300 rounded-lg w-full flex justify-between hover:bg-gray-300 hover:text-white transition-all duration-300'>
                        <div className=' flex item-center gap-3'>
                          <div className='w-10 h-10 font-bold text-gray-500 flex justify-center items-center border rounded-full overflow-hidden bg-white'>
                            {player.photo ? (<img src={player.photo} alt="" className="w-full h-full object-cover" />) : (player.name.charAt(0).toUpperCase())}
                          </div>
                          <div className='flex items-center gap-5'>
                        ...
                        <div className='flex justify-center items-center gap-1 text-red-500 font-semibold hover:text-red-700 hover:bg-red-100 hover:border-red-300 hover:border rounded-lg px-2 py-1 transition-all duration-300' onClick={() => handleDeletePlayer(player._id)}>
                          <Trash2 />
                        </div>
                      </div>
                    ))}
  ...
  return (
    <Router>
      <AppLayout 
        ...
        historyModalPlayer={historyModalPlayer}
        setHistoryModalPlayer={setHistoryModalPlayer}
        fetchData={fetchData}
